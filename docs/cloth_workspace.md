...
追加还必须防止骨号重新激活旧悬空边：如果当前组某个旧节点的 `noUp/noDown/noSide/noPoly` 等于本次新节点骨号，而该骨号在追加前不是当前组节点，那么这条引用原本无有效目标。合并前应把它清为 `4095`，否则新增袖骨可能让旧裙摆横向边突然落到袖子。`noFix` 允许引用组外锚骨，不参与该清理。

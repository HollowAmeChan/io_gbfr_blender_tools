...
- 创建和替换骨链只生成节点参数与拓扑连接，始终保留当前 CLP 的 `CLOTH_HEADER`。组级参数必须在 `CLP 求解` 面板中明确编辑；已有制作经验仅作为对应字段的说明文本，不会被创建工具自动写入。
...
- 制作分叉骨架时，尽量保持标准树状结构：各子支起始骨的 `head` 应在同一个分叉点重合，并保留通向分叉点的中间连接骨。不要为减少节点而删除中间连接骨，也不要把两支做成空间分离的独立根；游戏可能把两个动态根合并求解，导致整条动骨产生位置偏移。
- “删除所选”以用户当前选择为主体，不扩展普通后代。唯一自动清理的是拓扑尾端：所选骨的直接叶子子骨在当前 CLP 中存在节点，且该节点没有下游、上游指向所选节点或已经悬空。这里依据 Blender 父子关系和 CLP `up/down`，不依赖 `_end` 等命名后缀。为兼容旧数据，节点身份会按保存的精确骨骼引用、骨骼固定 ID 和当前导出 ID 解析，同一骨骼遗留的旧/新副本会一起处理。
- 创建 CLP 和模型联合导出前都会执行同一套 Cloth 骨号规范化：优先沿用骨骼当前固定 ID；没有固定 ID 时采用旧 CLP 中出现次数最多、数值最小的稳定编号。随后把 CLP 节点身份、`up/down/side/poly/fix` 和 CLH `p1/p2` 全部迁移到规范 ID，并合并同一 CLP 内同骨重复节点。该步骤发生在复制模型导出副本之前；模型导出器完成临时骨骼改名后还会返回实际使用的骨号映射，Cloth 以这份结果作最终权威校正再写 XML，而不是独立重算并假设两边一致。
...
旧版本创建的节点可能只有 `CLOTH_WK.no` 和 `bone_ref`，对应新增骨尚未固化 `gbfr_bone_id`。再次添加前应以现有节点记录为权威补写骨号，并释放未入 CLP 骨骼占用的冲突编号；否则第二次分配后旧节点仍在，但预览会因骨号不再对应而表现为第一串消失。
